...
namespace Security_network_module.DrawObjectCanvas
{
    class Coordinate_axes
    {
        ...
        public void Main_coordinate_axes()
        {
            ...
            Grid_coordinates(h* 1.86, h* 1.1, l* 0.9, l* 0.3, h* 0.076);
            //Графік №1
        ...
        public void Drawing_axes(double x0, double x1, double x2, double x3, double x4, double x5, double x6)
        {
            ...
            arr = new List<double> { Math.Abs(x0 + x5), x1, Math.Abs(x0), x1, Math.Abs(x0), Math.Abs(x1 - (x6 - x5)) };
            shapes.Draw_polyline(arr, 2, Colors.Black, false);

            // відмалювання стрілочок для графіків 1-2 вісь ОХ
            arr = new List<double> { Math.Abs(x0 + x4), x1 - (x5 - x4), Math.Abs(x0 + x5), x1, Math.Abs(x0 + x4), x1 + (x5 - x4) };
            shapes.Draw_polyline(arr, 2, Colors.Black, false);
            // відмалювання стрілочок для графіків 1-2 вісь ОY
            arr = new List<double> { Math.Abs(x0) - (x5 - x4), Math.Abs(x1 - (x6 - x4)), Math.Abs(x0), Math.Abs(x1 - (x6 - x5)), Math.Abs(x0) + (x5 - x4), Math.Abs(x1 - (x6 - x4)) };
            shapes.Draw_polyline(arr, 2, Colors.Black, false);
        }
    }
}
